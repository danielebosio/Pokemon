    ...
    if (caught.length === 0) {
        ...
        caught.forEach((pokemon, index) => {
            ...
            listItem.innerHTML = `
                ...
                <button class="remove-button" onclick="removePokemon(${index})" style="margin-left: 10px;">Rimuovi</button>
            `;
...
function displayPokemonStats(stats, types) {
    ...
    // Valore massimo per le statistiche
    const maxStatValue = 255;

    // Wrapper per le due colonne
    let statsHtml = '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">';

    stats.forEach((stat, index) => {
        // Calcola la percentuale della barra
        const percentage = Math.min((stat.base_stat / maxStatValue) * 100, 100);

        // HTML per ogni statistica
        statsHtml += `
            <div style="margin-bottom: 10px;">
                <span style="font-weight: bold;">${stat.stat.name.toUpperCase()}</span>
                <div style="position: relative; width: 100%; height: 20px; background: #e0e0e0; border-radius: 10px; overflow: hidden; margin-top: 5px;">
                    <div style="width: ${percentage}%; height: 100%; background: linear-gradient(to right, ${gradientColors[0]}, ${gradientColors[1] || gradientColors[0]});"></div>
                </div>
                <span>${stat.base_stat}</span>
            </div>
        `;
    ...
    // Inserisci il layout generato nel contenitore
...
function removePokemon(index) {
    console.log("Indice da rimuovere:", index);
    const caught = JSON.parse(localStorage.getItem('caughtPokemon')) || [];
    console.log("Lista Pokémon:", caught);
    
    if (index >= 0 && index < caught.length) {
        const removedPokemon = caught.splice(index, 1);
        console.log("Rimosso:", removedPokemon);
        localStorage.setItem('caughtPokemon', JSON.stringify(caught));
        showCaughtList();
        alert(`${removedPokemon[0].name} è stato rimosso dalla collezione.`);
    } else {
